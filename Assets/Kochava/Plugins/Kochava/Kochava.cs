...
public class Kochava : MonoBehaviour {
    ...
    private const string sdkVersionNumber = "4.0.0";
    private const string sdkNativeName = "Unity NET Store ";
    private const string sdkName = "Unity Store ";
    private const string sdkBuildDate = "2018-10-12T17:29:56Z";
    ...
    public static class Tracker
    {
        ...
        public static void Initialize()
        {
            ...
            androidNativeStart(_appGUID, _partnerName, _logLevel, _appAdLimitTracking, _sleep, _requestAttributionCallback, _intelligentConsentManagement, _manualManagedConsentRequirements, _dictionaryCustom);
#elif IOS
            LOG("Initializing Kochava native iOS library", LOG_INFO);
            iOSNativeStart(_appGUID, _partnerName, _logLevel, _appAdLimitTracking, _sleep, _requestAttributionCallback, _intelligentConsentManagement, _manualManagedConsentRequirements, _dictionaryCustom);
#elif WINDOWS_UWP
            LOG("Initializing Kochava native Windows Store (UWP) library", LOG_INFO);
            windowsUwpNativeStart(_appGUID, _partnerName, _logLevel, _appAdLimitTracking, _sleep, _requestAttributionCallback, _intelligentConsentManagement, _manualManagedConsentRequirements, _dictionaryCustom);
#else
        ...
        public static void SendEvent(Event kochavaEvent)
        {
            ...
            if (kochavaEvent == null) return;
#if ANDROID
            androidNativeSendKochavaEvent(kochavaEvent.getEventName(), kochavaEvent.getDictionary());
#elif IOS
            iOSNativeSendKochavaEvent(kochavaEvent.getEventName(), Json.ToString(kochavaEvent.getDictionary()));
#elif WINDOWS_UWP
	...
	public class ConsentPartner
	{
		...
		internal ConsentPartner(string name, string description, bool granted, long responseTime)
		{
			this.name = name;
...
#if IOS
            dictionary.Add("customEventName", eventName);
#endif
        }
    ...
    private static void androidNativeStart(string appGuid, string partnerName, int logLevel, bool appLimitAdTracking, bool sleep, bool retrieveAttribution, bool icm, bool icmManual, Dictionary<string, object> custom)
    {
        try
        {
            ...
            {                        
                ...
                //If sleep is enabled we will set it right after configuring.
                if(sleep)
                {
                    androidNativeSetSleepBool(sleep);
                }
    ...
    private static void androidNativeSendKochavaEvent(string eventName, Dictionary<string, object> dictionary)
    {
        // create the event
        ...
        if(dictionary != null)
        {
            ...
            object recData = "";
            object recSig = "";
            if (dictionary.TryGetValue("receiptDataFromGoogleStore", out recData) && dictionary.TryGetValue("receiptDataSignatureFromGoogleStore", out recSig))
            {
                myEvent.Call<AndroidJavaObject>("setGooglePlayReceipt", new AndroidJavaObject("java.lang.String", recData ?? ""), new AndroidJavaObject("java.lang.String", recSig ?? ""));
                dictionary.Remove("receiptDataFromGoogleStore");
                dictionary.Remove("receiptDataSignatureFromGoogleStore");
            }

            // build the event from the dictionary
            try
            {
    ...
    private static void iOSNativeStart(string appGuid, string partnerName, int logLevel, bool appLimitAdTracking, bool sleep, bool retrieveAttribution, bool icm, bool icmManual, Dictionary<string, object> custom)
    {
        ...
        config ["versionExtension"] = sdkName + sdkVersionNumber;
    ...
    [DllImport("__Internal")]
    private static extern void iOSNativeSendKochavaEvent(string eventName, string kochavaEventStringifiedDictionary);
    [DllImport("__Internal")]
    ...
    private static void windowsUwpNativeStart(string appGuid, string partnerName, int logLevel, bool appLimitAdTracking, bool sleep, bool retrieveAttribution, bool icm, bool icmManual, Dictionary<string, object> custom)
    {
#if !WINDOWS_UWP_SUPPORTED
    ...
    {
#if WINDOWS_UWP_SUPPORTED
        if(windowsTracker != null) windowsTracker.SendDeeplink(deepLinkURI ?? "");
#endif
    }
    ...
    private class NetworkManager : MonoBehaviour
    {
        ...
        private IEnumerator StartHttpRequest(NetworkRequest sourceRequest)
        {
            ...
            double wwwLoadTime = Time.time;

            // UNITY http call:
            WWW www = new WWW(sourceRequest.url, System.Text.Encoding.UTF8.GetBytes(postString));
            ...
            else
            {
                ...
                if (www.text != "")
                {
                    try
                    {
                        ...
                        if (sourceRequest.networkSuccessCallback(www.text))
                        {
    ...
    {
        var ev = new Item("deeplink");
        ev.data.Add("uri", deepLinkURI);
        if (sourceApplication.Length > 0) ev.data.Add("source_app", sourceApplication);
        intakeQueue.Add(ev);
        if (initialized) WakeController();
    }
        ...
        try
        {
            if (payloadInitResponse.ContainsKey("flags"))
            {
                ...
                if (flags.ContainsKey("kvtracker_wait"))
                {
                    ...
                    LOG("Tracker wait: " + profile.initWait + " s", LOG_DEBUG);
                }
    ...
    public void ProcessIntakeQueue()
    {
        ...
        while (intakeQueue.Count > 0)
        {
            ...
            if (profile.eventname_blacklist != null && Array.FindAll(profile.eventname_blacklist, s => s.Equals(intakeItem.properties.action)).Length > 0)
            {
