...
namespace Game.Addresssables.Utils
{
    ...
    public struct PooledBuffer : IDisposable
    {
        ...
        {
            if (this.Data != null)
                BoquBufferPool.Release(this.Data);
            this.Data = null;
        }
    }
    ...
    public static class BoquBufferPool
    {
        ...
        public static byte[] Resize(ref byte[] buffer, int newSize, bool canBeLarger)
        {
            ...
            byte[] newBuf = BoquBufferPool.Get(newSize, canBeLarger);
            Array.Copy(buffer, 0, newBuf, 0, Math.Min(newBuf.Length, buffer.Length));
            BoquBufferPool.Release(buffer);
