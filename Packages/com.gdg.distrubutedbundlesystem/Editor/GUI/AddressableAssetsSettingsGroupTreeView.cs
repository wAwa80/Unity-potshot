...
using static UnityEditor.Game.AddressableAssets.Settings.AddressablesFileEnumeration;
...
namespace UnityEditor.Game.AddressableAssets.GUI
{
    ...
    internal class AddressableAssetEntryTreeView : TreeView
    {
        ...
        static MultiColumnHeaderState.Column[] GetColumns()
        {
            ...
            retVal[counter].headerContent = new GUIContent("Group Name \\ Asset Path", "Address used to load asset at runtime");
        ...
        protected string CheckForRename(TreeViewItem item, bool isActualRename)
        {
            ...
            if (assetItem != null)
            {
                ...
                {
                    assetItem.displayName = assetItem.group.Name;
                    result = "Rename";
                }
        ...
        void MoveEntriesToNewGroup(object context)
        {
            ...
            var g = m_Editor.settings.CreateGroup(k.Key, false, true, k.Value.Schemas);
        ...
        internal void CreateNewGroup(object context)
        {
            ...
            {
                AddressableAssetGroup newGroup = m_Editor.settings.CreateGroup(groupTemplate.Name, false, true, null, groupTemplate.GetTypes());
                groupTemplate.ApplyToAddressableAssetGroup(newGroup);
            }
            else
            {
                m_Editor.settings.CreateGroup("", false, false, null);
                Reload();
            }
        }
    ...
    class AssetEntryTreeViewItem : TreeViewItem
    {
        ...
        public override string displayName
        {
            get
            {
                if (!isRenaming && group != null)
                {
                    return base.displayName + "[" + (group.GetSchema<BoquAssetGroupSchema>() != null ? group.GetSchema<BoquAssetGroupSchema>().GroupId : "0")+ "]";
                }
